...
## Data & Control Flow
`src/main.tsx` loads shared styles and calls `bootstrapOpenDucktorShell` with the desktop shell bridge factory. Build/release scripts shape CEF/Tauri inputs before the host is launched or packaged, while `src-tauri/` owns command registration, workspace/config updates, headless routing, and host service wiring.
