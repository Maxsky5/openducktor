...
## Design
The folder splits command registry/support, command helpers, SSE/event relays, and request handlers into focused modules so the web host can reuse the same host services. It mirrors the desktop command surface instead of inventing a separate runtime path.
