...
## Design/Patterns
Split into `src/` for Tauri-facing entrypoints and transport glue, `crates/` for hexagonal host logic, and `gen/` for generated bindings/schemas. Startup is fail-fast, blocking work is pushed off the UI thread, and lifecycle concerns stay isolated in `startup.rs`, `shutdown.rs`, `logging.rs`, and `app_state.rs`. Headless browser-backend routes and tests stay alongside the command bridge so the desktop and web-host surfaces remain aligned.
