...
## Design
The module is split into `registry`, `route`, `events`, and `state`. `registry` is a directory module with separate files for runtime kinds, capability contracts, descriptor validation, startup config, built-in OpenCode metadata, and registry mechanics. `RuntimeRoute` keeps the live connection shape explicit, and `RuntimeInstanceSummary` only carries live metadata for an active runtime.
