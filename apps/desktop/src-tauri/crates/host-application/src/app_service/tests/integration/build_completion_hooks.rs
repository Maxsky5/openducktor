...
#[test]
fn build_completed_validates_transition_before_running_post_complete_hooks() -> Result<()> {
    let _env_lock = lock_env();
    let root = unique_temp_path("build-completed-invalid-transition-no-hook");
    let repo = root.join("repo");
    init_git_repo(&repo)?;
    let worktree_base = root.join("builder-worktrees");
    let worktree = worktree_base.join("task-1");
    ...
    let error = service
        ...
        .expect_err("duplicate build completion should fail before cleanup hook execution");
    assert!(error
        .to_string()
        .contains("build_completed is only allowed from in_progress"));
