...
## Design
`mod.rs` defines the `AppRuntime` trait and registry container, while `open_code.rs` and `health_http.rs` provide the OpenCode runtime implementation and HTTP health/status client. Runtime setup, process lifecycle, and startup readiness live behind the OpenCode adapter boundary.
