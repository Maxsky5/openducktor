...
const navItems = [
  { to: "/open-repository", icon: FolderOpen, label: "Open Repository", requiresRepo: false },
...
export function AppShell(): ReactElement {
  const { runtimeCheck, activeRepo, isBusy, tasks, runs } = useOrchestrator();
    ...
    <div className="min-h-screen">
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-1 gap-4 p-4 md:grid-cols-[240px_1fr]">
        <aside className="rounded-2xl border border-slate-200/70 bg-white/85 p-4 shadow-sm backdrop-blur">
          ...
          <nav className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                <NavLink
                  key={item.to}
                  to={isDisabled ? "/open-repository" : item.to}
                  className={({ isActive }) =>
                    cn(
          ...
          <div className="mt-8 space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
        ...
        <section className="flex min-h-0 flex-col gap-4">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200/70 bg-white/85 px-4 py-3 shadow-sm backdrop-blur">
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <Badge variant={isBusy ? "warning" : "success"}>{isBusy ? "Working" : "Idle"}</Badge>
              {runtimeCheck ? (
                <Badge
                  variant={runtimeCheck.gitOk && runtimeCheck.opencodeOk ? "success" : "danger"}
                >
                  {runtimeCheck.gitOk && runtimeCheck.opencodeOk
                    ? "Runtime healthy"
                    : "Runtime issue"}
                </Badge>
              ) : (
                <Badge variant="secondary">Checking runtime</Badge>
              )}
            </div>
