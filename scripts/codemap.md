...
- `browser-dev.ts` is a compatibility shim that routes browser-mode development through the `@openducktor/web` workspace launcher.
...
- `browser-dev.ts` depends on the web workspace filter (`@openducktor/web`), which owns the Rust host launcher and browser transport.
