...
The check uses a prompt longer than 4,150 characters at 1280 × 577 and 390 × 320, in light and dark themes. It checks card and dialog image size, viewport bounds, aspect-ratio styling, collapsed prompt controls, full prompt retention after expansion, keyboard opening, Escape close, and focus return. It also checks visible file and background details, the generating skeleton, and failure guidance. Screenshots cover the card, expanded prompt, dialog, generating state, and failure state. Failure logs and a screenshot remain in the artifact directory when capture is available.

This check covers component layout. It does not claim real Codex generation or shell integration coverage. The normal frontend suite also checks prompt disclosure, session changes, and the short dialog header with Happy DOM.
