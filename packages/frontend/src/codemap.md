...
## Responsibility
Shared frontend application source for routes, shared components, feature orchestration, state/query layers, and shell-neutral runtime helpers.

## Design Patterns
Route-level lazy loading in `pages/`, feature composition in `components`, typed host-backed data access in `state/queries`, side-effectful mutations in `state/operations`, and a shell bridge in `lib/` for host capabilities.
