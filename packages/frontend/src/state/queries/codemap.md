...
## Data & Control Flow
Host reads are normalized here, then consumed by pages/components through `useQuery` or imperative preload paths. Mutations elsewhere invalidate these keys.

## Integration Points
`workspace.ts`, `tasks.ts`, `runtime.ts`, `documents.ts`, `checks.ts`, `git.ts`, `agent-sessions.ts`, and the host client/query client.
