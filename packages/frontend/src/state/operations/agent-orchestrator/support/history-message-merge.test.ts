...
describe("agent-orchestrator/support/history-message-merge", () => {
  ...
  test("reconciles a local accepted Codex image send with its loaded history item", () => {
    const attachmentPath = "/tmp/openducktor-local-attachments/uuid-image.png";
    const merged = mergedMessages(
      [
        {
          id: "runtime-user-confirmed",
          role: "user",
          content: `Review this image ${attachmentPath}`,
          timestamp: "2026-03-01T09:00:01.000Z",
          meta: {
            kind: "user",
            state: "read",
            parts: [
              { kind: "text", text: "Review this image" },
              {
                kind: "attachment",
                attachment: {
                  id: "codex-local-image:runtime-user-confirmed:1",
                  path: attachmentPath,
                  name: "image.png",
                  kind: "image",
                },
