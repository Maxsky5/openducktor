...
## Design Patterns
Message shaping, persistence, prompts, todos, assistant meta, runtime transcript merge helpers, and async side effects are split into small reusable utilities.
...
## Integration Points
`messages.ts`, `persistence.ts`, `todos.ts`, `assistant-meta.ts`, `core.ts`, `session-prompt.ts`, and runtime transcript merge helpers.
