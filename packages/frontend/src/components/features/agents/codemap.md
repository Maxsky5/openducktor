...
Agent feature UI: reusable chat surface plus Agent Studio header, session history, quick actions, workflow rail, session tabs, workspace sidebar, dev-server panel, git panel, right-panel composition, runtime attachment retry handling, and session-start modal entry points.
