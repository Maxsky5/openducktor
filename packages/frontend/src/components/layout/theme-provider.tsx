...
export function ThemeProvider({ children, defaultTheme = "light", ...props }: ThemeProviderProps) {
  ...
  const cancelSettingsSnapshotRefresh = useCallback(
    (): Promise<void> =>
      queryClient.cancelQueries({ queryKey: SETTINGS_SNAPSHOT_QUERY_KEY, exact: true }),
    [queryClient],
  );
    ...
    try {
      while (pendingThemeRef.current !== null) {
        ...
        try {
          ...
          await cancelSettingsSnapshotRefresh();
  ...
  }, [cancelSettingsSnapshotRefresh, writePersistedThemeToCache]);

  const selectTheme = useCallback(
    (newTheme: Theme): void => {
      pendingThemeRef.current = newTheme;
      void cancelSettingsSnapshotRefresh();
      applyThemeToDocument(newTheme);
      selectOptimisticTheme(newTheme);
      void persistPendingThemes();
    },
    [cancelSettingsSnapshotRefresh, persistPendingThemes],
  );
