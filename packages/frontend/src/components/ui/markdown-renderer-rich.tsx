...
import { isValidElement, type ReactElement, useEffect, useState } from "react";
...
const RICH_URL_TRANSFORM: UrlTransform = (url, _key, node) => {
...
export default function MarkdownRendererRich({
  ...
  const content = (frontMatter.kind === "valid" ? frontMatter.body : markdown).trim();
  if (!content) {
    return null;
  }
  ...
  return (
    <Markdown
      ...
      skipHtml
      urlTransform={RICH_URL_TRANSFORM}
