...
type CachedMarkdownElement = {
  element: ReactElement;
  linkPolicy: MarkdownLinkPolicy | undefined;
};

const elementCacheByComponents = new WeakMap<Components, Map<string, CachedMarkdownElement>>();

const getElementCache = (components: Components): Map<string, CachedMarkdownElement> => {
  const existing = elementCacheByComponents.get(components);
  if (existing) return existing;
  const created = new Map<string, CachedMarkdownElement>();
...
export const renderMarkdownElement = ({
  ...
  if (cached && cached.linkPolicy === linkPolicy) {
  ...
  if (markdown.length <= MAX_CACHED_MARKDOWN_LENGTH) {
    cache.set(markdown, { element, linkPolicy });
