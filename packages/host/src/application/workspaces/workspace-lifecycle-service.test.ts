...
describe("workspace lifecycle service", () => {
  ...
  test("closeWorkspace returns the catalog for an already closed workspace without inspection", async () => {
    ...
    const expected = catalog();
    const service = createService({
      activity: { inspect, releaseSessions: () => Effect.void },
  ...
  test("removeWorkspace records progress and blocks task work before deleting", async () => {
    const progress: Array<{ phase: string; pendingWorktreePath: string | null }> = [];
    const blockWorkspace = mock(() => {});
    const service = createService({
      admission: { ...createAdmissionDouble(), blockWorkspace },
      recordWorkspaceRemovalProgress: (input) =>
    ...
    expect(progress).toEqual([{ phase: "task_store", pendingWorktreePath: null }]);
    expect(blockWorkspace).toHaveBeenCalledWith({
      reason: "removal",
      repoPath: "/repos/ws",
      workspaceId: "ws",
  ...
  test("removeWorkspace rejects a registered task worktree from an earlier branch prefix", async () => {
    ...
    const service = createService({
      ...
      pathExists: (path) => Effect.succeed(path === "/old-base/task-1"),
      listWorktrees: () =>
        Effect.succeed([{ branch: "old/task-1-title", worktreePath: "/old-base/task-1" }]),
      removeWorktree,
      removeWorkspaceTaskAssets,
    });
    ...
    ).rejects.toThrow("Cannot classify registered worktree(s) for /repos/ws");
            ...
            openWorkspaces: [
              {
                ...
                defaultWorktreeBasePath: "/managed/other",
                effectiveWorktreeBasePath: "/managed/other",
              },
            ],
          }),
        ),
  ...
  test("removeWorkspace keeps a worktree that contains an incomplete-removal workspace", async () => {
    const removed: string[] = [];
    const service = createService({
      getWorkspaceCatalog: () =>
        Effect.succeed(
  ...
  test("removeWorkspace releases idle sessions before deleting workspace data", async () => {
    const calls: string[] = [];
    const releaseSessions = mock(() =>
      Effect.sync(() => {
        calls.push("release-sessions");
      }),
    );
    const service = createService({
      activity: {
    ...
    expect(releaseSessions).toHaveBeenCalledWith("/repos/ws");
    expect(calls).toEqual(["release-sessions", "remove-assets"]);
  });
  ...
  test("removeWorkspace resumes after a recorded worktree was deleted", async () => {
    ...
    const service = createService({
      ...
      pathExists: (path) => Effect.succeed(path === "/managed/ws/task-2"),
      listWorktrees: () =>
        Effect.succeed([
          { branch: "odt/task-1", worktreePath: "/managed/ws/task-1" },
    ...
    expect(result.removedWorktrees).toEqual(["/managed/ws/task-2"]);
  });
  ...
  test("removeWorkspace rejects a pending path that is no longer a registered worktree", async () => {
    const removeWorktree = mock(() => Effect.void);
    const removeWorkspaceTaskAssets = mock(() => Effect.void);
    const pendingWorktreePath = "/managed/ws/task-1";
    const service = createService({
      getRepoConfig: () =>
        Effect.succeed(
          repoConfig({
            removal: removalRecord({ pendingWorktreePath }),
          }),
        ),
      beginWorkspaceRemoval: () => Effect.succeed(removalRecord({ pendingWorktreePath })),
      taskStore: createTaskStoreDouble([taskCard("task-1")]),
      listWorktrees: () => Effect.succeed([]),
      removeWorktree,
      removeWorkspaceTaskAssets,
  ...
  test("removeWorkspace reports completed worktrees when a later worktree fails", async () => {
    let removalCount = 0;
    const pendingPaths: Array<string | null> = [];
    const service = createService({
      taskStore: createTaskStoreDouble(
      ...
      recordWorkspaceRemovalProgress: (input) =>
        Effect.sync(() => {
          pendingPaths.push(input.pendingWorktreePath);
        }),
    });
    ...
    expect(pendingPaths.at(-1)).toBe("/custom/worktrees/task-2");
  });
