...
export type LocalAttachmentService = {
  stage(input: unknown): Promise<StagedLocalAttachment>;
  resolve(input: unknown): Promise<ResolvedLocalAttachment>;
};
...
const parseStageInput = (input: unknown): { name: string; base64Data: string } => {
  const record = requireRecord(input, "Local attachment stage input");
  const name = requireString(record.name, "Attachment name");
  const base64Data = requireString(record.base64Data, "Attachment payload");
  return { name, base64Data };
};

const parseResolveInput = (input: unknown): { path: string } => {
  const record = requireRecord(input, "Local attachment resolve input");
...
export const createLocalAttachmentService = (
  ...
  async stage(input) {
    const { name, base64Data } = parseStageInput(input);
    ...
    const fileName = `${randomUUID()}-${sanitizeAttachmentFilename(name)}`;
  ...
  async resolve(input) {
    const { path } = parseResolveInput(input);
