...
export type SystemDiagnosticsService = {
  runtimeCheck(forceRefresh?: unknown): Promise<RuntimeCheck>;
  beadsCheck(repoPath: unknown): Promise<BeadsCheck>;
  systemCheck(repoPath: unknown): Promise<SystemCheck>;
};
...
const requireBooleanLike = (value: unknown, label: string): boolean => {
...
export const createSystemDiagnosticsService = ({
  ...
  const runtimeCheck = async (forceRefresh?: unknown): Promise<RuntimeCheck> => {
    const force = requireBooleanLike(forceRefresh, "runtime_check force");
  ...
  const beadsCheck = async (repoPath: unknown): Promise<BeadsCheck> => {
  ...
  const systemCheck = async (repoPath: unknown): Promise<SystemCheck> => {
