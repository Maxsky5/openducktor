...
describe("createTaskService build start worktree handling", () => {
  ...
  test("rejects an existing task worktree owned by another workspace before setup", async () => {
    const calls: unknown[] = [];
    ...
    expect(calls).not.toContainEqual(expect.objectContaining({ type: "currentBranch" }));
