...
`,
    ...
    await expect(port.listTasks({ repoPath: "/repo", doneVisibleDays: 1 })).resolves.toHaveLength(
      1,
    );
    await expect(port.listTasks({ repoPath: "/repo", doneVisibleDays: 2 })).resolves.toHaveLength(
      1,
    );
  ...
  test("reuses fresh task lists and invalidates them after metadata mutations", async () => {
    ...
    const first = await port.listTasks({ repoPath: "/repo" });
    const firstTask = first[0];
    if (!firstTask) {
      throw new Error("expected cached task");
    }
    firstTask.title = "Mutated by caller";
    const cached = await port.listTasks({ repoPath: "/repo" });
    await port.setSpecDocument({ repoPath: "/repo", taskId: "task-1", markdown: "# Spec" });
    const refreshed = await port.listTasks({ repoPath: "/repo" });
