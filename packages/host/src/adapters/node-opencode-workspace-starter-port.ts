...
import { resolveOpencodeBinary } from "./node-runtime-binary-resolution";
...
export const createNodeOpenCodeWorkspaceStarterPort = ({
...
}: CreateNodeOpenCodeWorkspaceStarterPortInput): RuntimeWorkspaceStarterPort => ({
  async startWorkspaceRuntime(input): Promise<RuntimeWorkspaceHandle> {
    ...
    if (input.runtimeKind !== "opencode") {
      throw new Error(
        `Node OpenCode workspace starter does not support runtime kind ${input.runtimeKind}.`,
      );
    }
