...
describe("createNodeCodexWorkspaceStarterPort", () => {
  ...
  test("fails fast when the MCP bridge connection is not configured", async () => {
    const starter = createNodeCodexWorkspaceStarterPort({
      systemCommands: createSystemCommands(),
      codexAppServer: createInMemoryCodexAppServerPort(),
      mcpCommand: ["mcp-bin"],
    });
  ...
  test("starts a Codex app-server runtime, registers transport, and stops it", async () => {
    ...
    try {
      ...
      const codexAppServer = createInMemoryCodexAppServerPort();
      const starter = createNodeCodexWorkspaceStarterPort({
        systemCommands: createSystemCommands(),
        codexAppServer,
        codexBinary,
