...
const queuePendingBackgroundTaskResult = (
  ...
  const pending =
    runtime.session.pendingBackgroundTaskResultsByExternalSessionId.get(externalSessionId);
  const next = [
    ...(pending?.filter((entry) => entry.part.partId !== part.partId) ?? []),
    { part, timestamp },
  ];
  runtime.session.pendingBackgroundTaskResultsByExternalSessionId.set(externalSessionId, next);
};
