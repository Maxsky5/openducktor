...
describe("Azure DevOps repository identity", () => {
  test("normalizes equivalent service addresses at the identity boundary", () => {
    ...
    const canonicalRepository = { ...repository, serviceUrl: "http://ado.example/installation" };

    expect(canonicalAzureDevOpsServiceUrl(repository.serviceUrl)).toBe(
      "http://ado.example/installation",
    );
    ...
    expect(azureDevOpsCollectionUrl(repository)).toBe(
      "http://ado.example/installation/DefaultCollection",
    );
    expect(
      azureDevOpsResolvedRepositoryIdentity({
        ...repository,
      ...
      "azure_devops::http://ado.example/installation/DefaultCollection::project-id::repository-id",
    );
  });
