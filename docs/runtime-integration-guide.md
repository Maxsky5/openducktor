...
Adapters send `image_generation_settled` through the transcript stream when a turn ends, even if no image start event was observed. A turn ID limits the outcome to that turn. A session-wide outcome covers unfinished images up to the event timestamp. The frontend retains this context in transient session state and applies it to history that arrives later. These events do not change session activity or pending input. Once a runtime supplies image settlement events, those events own image outcomes; generic session events do not replace them.
