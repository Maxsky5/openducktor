...
Implement `RuntimeSessionImportPort` to list metadata for existing root conversations. Use native paging when available and treat page tokens as opaque; the port must not depend on a native cursor shape. Listing and `getMetadata` must not read a transcript, send a prompt, or add a session to live state.

`openForImport` must check the exact source and keep its original ID, directory, and available settings. It returns a `RuntimeSessionImportHandle`. The host saves the workspace association before it calls `commit` on that handle. `dispose` closes any uncommitted native resources. A failed live admission must leave the saved association available for retry.
