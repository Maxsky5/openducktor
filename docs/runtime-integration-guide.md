...
The shared/core boundary keeps slash commands as structured message parts. OpenDucktor-owned system commands are filtered by composer scope and are available only when their prerequisites are present; `/compact` is reserved for existing OpenCode and Codex sessions and takes precedence over matching runtime commands or reusable prompts. The OpenCode adapter converts ordinary leading slash-command tokens plus trailing text into the runtime's native `session.command` request, where the trailing text becomes the command `arguments`. The system `/compact` exception calls stable `session.summarize` with the selected session model and accepts no trailing or structured input. Codex exposes only the system `/compact` command and calls `thread/compact/start`; its standard `contextCompaction` item notifications remain the start/completion authority. If a draft cannot be represented faithfully by the runtime-specific endpoint, the adapter fails explicitly instead of flattening the structured command back into plain text.
