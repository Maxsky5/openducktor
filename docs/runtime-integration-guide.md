...
The Codex adapter uses an explicit `CodexImageGenerationEnd` scope. A turn end requires a `turnId`. A session end cannot claim a confirmed interruption. Runtime and request failures use the session scope; native turn completion uses the turn scope.
...
Adapters send `image_generation_settled` through the transcript stream when a turn ends, even if no image start event was observed. A turn ID limits the outcome to that turn. A session-wide outcome covers unfinished images up to the event timestamp and images whose history timestamp is unknown or approximate. A synthesized history load time does not prove that a generation started after the outcome. The adapter and frontend retain this context in transient session state and apply it to history that arrives later. A confirmed `image_generation_turn_started` event identifies a subsequent real turn, which remains usable without reviving an ended turn. These events do not change session activity or pending input. Once a runtime supplies image settlement events, those events own image outcomes; generic session events do not replace them.

Core owns the pure image lifecycle policy in `packages/core/src/services/agent-image-generation-lifecycle.ts`. Codex and the frontend use it for turn starts, turn ends, interruption priority, and session cutoffs. Each owner selects the turns covered by a session end and retains its own transient data. Live Codex items use exact turn outcomes; history and frontend messages also use timestamp cutoffs. The frontend keeps the latest session cutoff when events arrive out of order.
...
Codex 0.153.0 supplies structured image failure details for usage limits. For other extension failures, it emits an image item with an empty result and sends the error message to the agent through `RespondToModel`; see the [native failure path](https://github.com/openai/codex/blob/rust-v0.153.0/codex-rs/ext/image-generation/src/tool.rs#L190-L209). The card states when the image result has no reason and directs the user to ask the agent to explain the failure. Do not invent a cause or treat image result bytes as error text.
