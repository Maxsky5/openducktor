...
1. `useTaskQueryReadModel` observes `repoTaskDataQueryOptions`. The initial stream snapshot or repository load fills the shared query cache before the render query can refetch it. Manual and event refreshes use `TaskViewSync`.
