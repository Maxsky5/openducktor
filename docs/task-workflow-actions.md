...
The task detail sheet shows the same available actions on every surface, such as Kanban, Agent Studio, and the agent chat viewer. The task workflow actions context supplies the action handlers. A surface can override the edit handler for its own routing, but it cannot remove an action.
