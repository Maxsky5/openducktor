...
The host CORS allowlist includes the configured frontend origin, its hostname without a trailing dot, and loopback origins at the frontend listening port. Browser requests from other origins get a `403`. CORS permission does not make cross-site session cookies work. With a remote `--external-url`, open that URL, not a local frontend URL. The launcher displays only the remote URL in this mode. Use the configured hostname consistently; accepting both dotted and undotted origins does not make their cookies interchangeable. The terminal WebSocket checks the origin, the session cookie, and the WebSocket subprotocol. The `/shutdown` route requires the launcher control token, which the browser never receives.
