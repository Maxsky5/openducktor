...
The host allows only the configured frontend origin in CORS, plus the loopback variants of its port for on-machine testing. Browser requests from other origins get a `403`. The terminal WebSocket checks the origin, the session cookie, and the WebSocket subprotocol. The `/shutdown` route requires the launcher control token, which the browser never receives.
