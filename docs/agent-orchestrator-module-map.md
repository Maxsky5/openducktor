...
- Sessionless composer submission validates the exact model selection and catalog, then starts a fresh task workflow session through `startSessionForMessage`. It sends typed draft parts without a start modal or generated kickoff.
- Explicit launches resolve the kickoff through `session-start-kickoff.ts` before the shared modal can confirm. The modal edits one optional string and owns no task or settings reads. The workflow validates the baseline before pre-start mutations and sends the confirmed text once.
