...
- `projectObservedSessionActivity` in `session-read-model/agent-session-live-projection.ts` and `projectSessionTranscriptActivity` in `session-read-model/agent-session-live-activity.ts` state the status rules. The workspace rail activity projection in `features/workspace-activity/` reuses both, so a rail tile badge and an Agent Studio session cannot disagree about the same live session. Change the rule in these two functions, not in a copy.
