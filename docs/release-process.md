...
- merges the macOS per-architecture updater manifests into one canonical `latest-mac.yml`
...
The packaged desktop updater uses GitHub Releases as the stable/latest feed for v1. There are no release channels, prerelease updater controls, nightly feeds, or persisted updater preferences in v1.
...
- updater metadata files generated by Electron Builder: `latest.yml`, `latest-mac.yml`, and `latest-linux.yml`
- generated blockmaps such as `*.blockmap` where Electron Builder produces them

The release workflow stages updater metadata next to installable artifacts and fails if a staged release is missing required `latest*.yml` files. macOS arm64 and x64 builds initially produce separate manifests; the publish job merges those into the canonical `latest-mac.yml` before uploading assets to the draft release.
