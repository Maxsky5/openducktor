...
tester.run("anti-slop/no-unsafe-dictionary-type", noUnsafeDictionaryTypeRule, {
  valid: [
    ...
    "type Empty = Omit<Record<`${number}`, object>, `${string}`>;",
    "type Empty = Omit<Record<`pre-${number}-${string}`, object>, `pre-${string}`>;",
    'type FiniteRemap = { [K in string as "only"]: object };',
    "type EmptyRemap = { [K in string as never]: object };",
    {
  ...
  invalid: [
    ...
    { code: "type A = Record<string, object & unknown>;", errors: [error] },
