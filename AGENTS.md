...
- In Bun tests, NEVER call `mock.restore()` from `beforeAll`, `beforeEach`, or other setup-time paths. `mock.restore()` is process-wide in Bun and can wipe unrelated mocks from concurrently running test files. If one test needs the real implementation while another file keeps a module mock, import the real module through an isolated path or remove the module mock instead of doing setup-time global restore.
