...
- In Bun tests, register `mock.module(...)` in scoped setup (`beforeAll`/`beforeEach`) only when you need a module seam, and do not rely on `mock.restore()` for module cleanup. `mock.restore()` is process-wide in Bun and can wipe unrelated mocks from concurrently running test files even from `afterAll`/`afterEach`.
- In Bun tests, if one file needs the real implementation while another keeps a module mock, restore the exact module id by remocking it back to the real implementation or import the real module through an isolated path. Do not use process-wide `mock.restore()` for module cleanup in shared suite runs.
